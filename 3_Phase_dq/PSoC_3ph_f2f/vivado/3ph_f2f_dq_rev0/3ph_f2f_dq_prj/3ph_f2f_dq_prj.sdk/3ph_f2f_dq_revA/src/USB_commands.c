...
/****************************************************************************
 * Phase generator command decoding
 ****************************************************************************/
void phGenCMDdecode(u8 ID, u8 *cargoPtr, phaseGen *phGenInst, phaseGenCfg *CfgData){
	...
	case PHG_CFG_SET:
		...
		memcpy(CfgData, cargoPtr, sizeof(phaseGenCfg));
		phaseGenSetConfiguration(phGenInst, CfgData);
		break;
	}
}

/****************************************************************************
 * PLL command decoding
 ****************************************************************************/
void pllSysCMDdecode(u8 ID, u8 *cargoPtr, pllSys *pllSysInst, pllSysCfg *CfgData){
	...
	case PLL_CFG_SET:
		/* Set new control configuration */
		memcpy(CfgData, cargoPtr, sizeof(pllSysCfg));
		pllSysSetConfiguration(pllSysInst, CfgData);
		break;
	}
}

/****************************************************************************
 * VSI command decoding
 ****************************************************************************/
void vsiCMDdecode(u8 ID, u8 *cargoPtr, VSIControl *vsiInst, VSIControlCfg *CfgData){
	switch (ID){
	case VSI_CFG_GET:
		/* Get the current configuration */
	...
	case VSI_CFG_SET:
		/* Set new control configuration */
		memcpy(CfgData, cargoPtr, sizeof(VSIControlCfg));
		vsiControlSetConfiguration(vsiInst, CfgData);
		break;
	}
}
